...
### K) เปรียบเทียบ trend indicator (โบ้ถาม: SAR? EMA9?) — 1H, 3 ช่วง
| ind | flips/ปี | ใน grid framework (ret/DD 3 ช่วง) | verdict |
|---|---|---|---|
| EMA50 | ~450 | **9.7 / 9.7 / 14.8 ดีสุดครบ** | ✅ ถ้าจะใช้ regime filter ให้ใช้ตัวนี้ |
| EMA200/SMA200 | ~200 | 9.5/3.5/13.2 vs 8.9/3.7/6.0 | ✅ เสถียร (EMA200 ≥ SMA200) |
| SAR 0.02/0.2 | ~480 | 7.7/3.3/14.7 | ⚠️ แกว่ง — ดีตอนเทรนด์เรียบ ตายตอน chop |
| **EMA9** | **~1,150** | แพ้ทุกช่วง, ทิศขัด fwd return ทุกช่วง | ❌ noise follower ไม่ใช่ trend |
หมายเหตุ: เทียบบน coarse 1H (ranking น่าเชื่อ, absolute ไม่ — grid ยังโดน 1M penalty อยู่ดี)
และ Breakout ตัวหลักไม่ต้องการ trend filter เพิ่ม (Donchian คือ trend detection ในตัว)

**K-followup (1M replay 22-26, grid x1.5 close-on-flip, cost จริง):** EMA50 ยืนยันบน 1M —
ret/DD 2.09 vs SMA200 0.54 (ดีขึ้น 4 เท่า), EMA9 0.30 (ยืนยันขยะ)
→ **best-possible trend-martingale = EMA50 + close-on-flip + x1.5** แต่ ret/DD 2.1 ยังแพ้ Breakout (5-12)
= ปิดประเด็น martingale ทุกสาขาแล้ว: เวอร์ชันดีสุดที่หาได้ก็ยังไม่คุ้มใช้

**EMA sweep เต็ม (9-300, เลือกด้วย worst-case ข้าม 3 regime):** plateau จริงอยู่ที่ **EMA 34-75**
(coarse min ret/DD 5.7-9.6; 1M: 1.85-2.09 เกาะกลุ่ม) — EMA50 แกนกลาง, EMA20 หลุด plateau บน 1M (0.78),
EMA100+ เสื่อม, EMA200 = 3.5 แพ้โซนนี้ 3-4 เท่า
→ ความรู้ติดโปรเจกต์: trend definition ที่ดีของทอง 1H อยู่แถว EMA34-75 (อาจมีประโยชน์กับงานอื่นเกิน grid)
